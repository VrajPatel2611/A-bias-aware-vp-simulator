...
// ── parse markdown ─────────────────────────────────────────────────
const src = fs.readFileSync("SYSTEM_DESIGN.md", "utf8").split("\n");
const body = [];
let i = 0;
let inTocSection = false;

// skip the front-matter block (title + meta) — rendered on the cover page
while (i < src.length && !/^## How to read/.test(src[i])) i++;
...
const cover = [
  ...
  new Paragraph({
    spacing: { after: 120 },
    children: [new TextRun({ text: "SYSTEM DESIGN & ENGINEERING SPECIFICATION", bold: true, size: 19, color: AMBER, characterSpacing: 60 })],
  }),
  new Paragraph({
    spacing: { after: 100 },
    children: [new TextRun({ text: "VPSim", bold: true, size: 76, color: INK, font: "Georgia" })],
  }),
  new Paragraph({
    spacing: { after: 340 },
    children: [new TextRun({ text: "Bias-Aware Virtual Patient Simulator", size: 32, color: TEAL, font: "Georgia" })],
  }),
  new Paragraph({
    spacing: { after: 420 },
  ...
  ...[
    ...
    ["Scope", "Research prototype → deployable multi-tenant platform"],
...
Packer.toBuffer(doc).then(b => {
  fs.writeFileSync("VPSim_System_Design.docx", b);
  console.log("Written VPSim_System_Design.docx —", (b.length / 1024).toFixed(0), "KB");
});
