...
// ── parse markdown ─────────────────────────────────────────────────
const [,, IN, OUT, TITLE, SUB, STATUS, PARTOF] = process.argv;
...
const cover = [
  ...
  new Paragraph({
    spacing: { after: 120 },
    children: [new TextRun({ text: SUB, bold: true, size: 19, color: AMBER, characterSpacing: 60 })],
  }),
  new Paragraph({
    spacing: { after: 100 },
    children: [new TextRun({ text: TITLE, bold: true, size: 56, color: INK, font: "Georgia" })],
  }),
  new Paragraph({
    spacing: { after: 340 },
  ...
  ...[
    ...
    ["Status", STATUS],
    ["Part of", PARTOF || "Build specification — 6 documents + ADR log"],
...
Packer.toBuffer(doc).then(b => {
  ...
  console.log("Written " + OUT + " —", (b.length / 1024).toFixed(0), "KB");
});
